...
class CourseController extends  ToeflController{
    ...
    public function actionCourse(){
        ...
        $model = new Content();
        $data = $model->courseSearch(155,$category,$type,$order,$page);
        ...
        $this->type = 1;
        return $this->exitData($data,$dataType,"course",2);
    }
    ...
    public function actionClassDetails(){
        ...
        if($sign->pid == 0){
            $data =  $model->getClass(['fields' => 'sentenceNumber,price,originalPrice,numbering,duration,cnName','where' =>"c.pid=$id",'pageSize' => 1]);
            if(count($data) == 0){
            ...
            $data =  $model->getClass(['fields' => 'sentenceNumber,price,originalPrice,numbering,duration,cnName','where' =>"c.id=$id",'pageSize' => 1]);
    ...
    public function actionHotClass(){
        $dataType = Yii::$app->request->get('data-type','arr');
        $model = new Content();
        $class = $model->getClass(['fields' => 'numbering','category' =>"155,163,182",'pageSize' => 4,'where' => "c.pid=0"]);
