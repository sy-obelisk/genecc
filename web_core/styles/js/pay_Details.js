...
function ck(e,n){
    ...
    $("#num").val(num);
    var price = Number($("#oldprice").val());//原价
    var now = Number($("#hidenLD").val());//已有的豆
    var zongjia = Number(adjust(price*num,$("#adjust").val()));
    ...
    if(zongjialeidou<now){
        ...
        var yingfu = parseFloat(Number(zongjia-(zongjialeidou/100)));
        var floats =Number(Math.round(yingfu*100)/100);
        $("#yingfu").html(floats)
    }
    if(zongjialeidou>now){
        $("#jifenMoney").val(now);
...
function adjust(a,e){
   var zongjia = (Math.round(Number(a)*100)+Math.round(Number(e)*100))/100;
    return zongjia;
