...
function reglogin(e) {
    var tel = $("#phone").val();
    if (tel == '' || !tel.match(/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/)) {
        $("#phone").focus();
        return false;
    } else {
        $("#login_register").hide();
        $("#lu_fly").show();
        console.log( $("#rl_password").val());
        $.ajax({
            url: 'index.php?web/webapi/register', // 跳转到 action
            data: {
                username: "GMAT"+Date.parse( new Date()),
                phone: $("#phone").val(),
...
function EmailReg(e) {
    ...
    if (tel == '' || !tel.match(/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,4}$/)) {
        $("#regEmail").focus();
        return false;
    } else if (yanz && yanzhen) {
        $.ajax({
...
function login(e) {
    ...
    $("#lu_fly").show();
    $.ajax({
        url: '/index.php?web/webapi/login', // 跳转到 action
        data: {
        ...
        success: function (data) {
            if (data.code == 1) {
                setCookie($("#reg_rem"), $("#log_username"), $("#log_password"));
...
//写入cookies
function setCookie(ob, username, password) {
    if (ob && ob.is(':checked')) {
...
function logout() {
    $.ajax({
        url: 'index.php?web/webapi/logout', // 跳转到 action
        type: 'post',
        cache: false,
        dataType: 'json',
        success: function (data) {
...
function get_Phonecode(e) {
   var Phonenum = $("#" + $(e).attr('phone')).val();
   var Reg = $("#" + $(e).attr('Reg')).val();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code == 1) {
                    ...
                    alert(data.message);
                    $(e).parent("div").prev("div").find(".miss_login").html(data.message).show();
                }
            },
            error: function () {
                alert("网络通讯失败,请检查网络连接，或者联系网站管理员！");
...
function qstlist(self, pageNumber, pageSize) {
    ...
    $.ajax({
        url: 'index.php?web/webapi/gettmlt', // 跳转到 action
        data: {
            'section': $('#section').val(),
            'level': $('#level').val(),
            'origin': $('#origin').val(),
        ...
        success: function (data) {
            if (data.code == 1) {
                ...
                $(data.lidata).each(function (i, row) {
                    str = str + '<li>';
                    //str = str + '<a href="/question/' + row.questionid + '-' + row.section + '.html"><span class="hy_lx_yf_title">[' + row.section + ']-' + row.questionid + '-' + row.twbname + '</span></a>';
                    str = str + '<a href="/question/' + row.questionid + '.html" target="_blank"><span class="hy_lx_yf_title">[' + row.section + ']-' + row.questionid + '-' + row.twbname + '</span></a>';
                    str = str + '<div class="hy_lx_p"><a href="/question/' + row.questionid + '.html" target="_blank">' + row.question + '</a></div>';
        ...
        success: function (data) {
            if (data.code == 1) {
                ...
                $(data.lidata).each(function (i, row) {
                    ...
                    str = str + '<a href="/beikao/' + row.contentid + '.html"><h3>' + row.contenttitle + '</h3></a>';
                    str = str + '<span>' + time + '</span>';
                    str = str + '<p>' + row.abstract + '</p>';
                    str = str + '</div>';
                    str = str + '<div style="clear: both"></div>';
                    str = str + '<a href="/beikao/' + row.contentid + '.html" class="readAll">阅读全文 <i class="fa fa-angle-double-right"></i></a>';
                    str = str + '</li>';
                    //}
                });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.code == 1) {
                ...
                $(data.lidata).each(function (i, row) {
                    str += '<li>' +
                        '<a href="/question/' + row.questionid + '-' + row.twoobjecttype + '-' + row.sectiontype + '.html">' +
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.code == 1) {
                ...
                $(data.lidata).each(function (i, row) {
                    var hyclass;
                    if (typeof(row.qanswer.qanswer) != 'undefined' && typeof(row.questionanswer) != 'undefined' && row.qanswer.qanswer == row.questionanswer) {
...
function loginw(_self) {
    window.location.href="http://login.viplgw.cn/cn/index?source=1&url="+window.location.href;
}
